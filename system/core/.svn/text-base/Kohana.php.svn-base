...
final class Kohana {
	...
	public static function setup()
	{
		...
		if( isset($_SERVER['HTTP_HOST']) && $_SERVER['HTTP_HOST'] != self::config('config.site_domain') ){
			$lang = str_replace( '.'.self::config('config.site_domain') , "", $_SERVER['HTTP_HOST']);
			if( array_key_exists($lang, self::config('locale.languages')) ){
				$locales = self::config('locale.languages.' . $lang );
				self::config_set('config.base_site_domain', self::config('config.site_domain') );
				self::config_set('config.site_domain', $_SERVER['HTTP_HOST']);
				self::config_set('core.site_domain', $_SERVER['HTTP_HOST']);
				self::$lang_code = $lang;
				self::config_set('locale.language',  $locales);
			}
			
		}else {
			$codes = array_keys(self::config('locale.languages'));
			self::$lang_code = $codes[0]; // first is default language
			self::config_set('config.base_site_domain', self::config('config.site_domain') );
		}
