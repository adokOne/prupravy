...
class arr_Core {
	...
	public static function ORM_object_to_array($objects){
		
		$array = array();
		
		foreach ($objects as $object){
			
			$array[$object->id] = $object->as_array();
		}
		
		return $array;
	}
	/**
	 * возвращает масив в виде дерева
	 *
	 * @param array $a		масив
	 * @param string $p		название индекса парент ид
	 * @param string $c		название индекса чилдов
	 * @return unknown
	 */
	public static function array_stack (&$a, $p = '@parent', $c = '@children'){
		$l = $t = array();
