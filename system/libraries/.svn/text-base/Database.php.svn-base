...
class Database_Core {
	 ...
	 * @return  Database_Core        This Database object.
	 */
	public function orderby($orderby, $direction = NULL, $do_not_escape_this_fuckin_value_and_use_column_only = false )
	{
		if ( ! is_array($orderby))
		{
		...
		foreach ($orderby as $column => $direction)
		{
			...
			if($do_not_escape_this_fuckin_value_and_use_column_only)
				$this->orderby[] = $column;
			else
