...
class Image_Core {
    ...
    public function textWatermark($text, $fontsize, $left = null, $top = null){
        if(is_null($left)){
            $left = $this->width / 2;
        }
        if(is_null($top)){
        ...
        $this->actions['textWatermark'] = array('text' => $text, 'left' => (int)$left, 'top' => (int)$top, 'fontsize' => $fontsize);
        return $this;
    }

    public function center_image($width, $height){
        
        $this->actions['center_image'] = array('width'=>$width, 'height'=>$height);
        return $this;
    }
} // End Image