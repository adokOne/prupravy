...
Ext.ux.UploadDialog.BrowseButton = Ext.extend(Ext.Button, {
	...
	createInputFile: function(){ 
		...
		this.input_file = Ext.DomHelper.append(button_container, {
			...
			style: 'position: absolute; display: block; border: none; cursor: pointer; left:-55px;top:0;'
		}, true);

		var button_box = button_container.getBox();

		//this.input_file.setStyle('font-size', (button_box.width * 0.5) + 'px');
                this.input_file.setStyle('font-size', '16px');
...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
	...
	updateRecordState: function(data){
		if ('success' in data.response && data.response.success) {
			...
			data.record.set('note', data.response.msg || data.response.error || this.i18n.note_upload_success);
		}
		else {
			data.record.set('state', Ext.ux.UploadDialog.FileRecord.STATE_FAILED);
			data.record.set('note', data.response.msg || data.response.error || this.i18n.note_upload_error);
		}
...
*/
	...
	upload_btn_stop_text: 'Отмена',
	upload_btn_start_tip: 'Upload queued files to the server.',
	upload_btn_stop_tip: 'Остановить загрузку.',
	close_btn_text: 'Закрыть',
	close_btn_tip: 'Close the dialog.',
	progress_waiting_text: 'Ожидайте...',
	progress_uploading_text: 'Загружено: {0} из {1} файлов.',
	error_msgbox_title: 'Ошибка',
	permitted_extensions_join_str: ',',
	err_file_type_not_permitted: 'Выбранный тип файла не разрешен.<br/>Вы можете загрузить только файлы следующих типов: {1}',
	note_queued_to_upload: 'Queued for upload.',
	note_processing: 'Загрузка...',
	note_upload_failed: 'Server is unavailable or internal server error occured.',
	note_upload_success: 'OK.',
	note_upload_error: 'Upload error.',
	note_aborted: 'Отменено пользователем.'
}
