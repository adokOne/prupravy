...
class url_Core {
	 ...
	 * @return  string
	 */
	public static function base($index = FALSE, $protocol = FALSE, $subdomain = FALSE, $lang = FALSE, $skip_domain = FALSE)
	{
		if ($protocol == FALSE)
		{
        ...
        $lang = $lang !== FALSE ? ($lang ? $lang.'/' : '') : Router::$cur_lang;
        $subdomain = $subdomain ? Kohana::config('multi_lang.domain_names.'.$subdomain) : $subdomain;
        $site_domain = $subdomain ? (Kohana::config('config.has_subdomains') ? $subdomain.'.'.rtrim($site_domain, '/').'/'.$lang : rtrim($site_domain, '/').'/'.$lang.($skip_domain ? '' : $subdomain)) : rtrim($site_domain, '/').'/'.$lang;
	 ...
	 * @return  string
	 */
	public static function site($uri = '', $protocol = FALSE, $subdomain = FALSE)
	{
		...
		// Concat the URL
		return url::base(TRUE, $protocol, $subdomain).$path.$query.$fragment;
	}
