...
class Image_GD_Driver extends Image_Driver {
	...
	public function addborder($image = 'img/carousel_border.png'){
		
		$width = Kohana::config('events.img.0.width');
		$height = Kohana::config('events.img.0.height');
		$border = imagecreatefrompng(DOCROOT . $image);
		imagealphablending($border, true); // setting alpha blending on
		imagesavealpha($border, true);
		imagesavealpha($this->tmp_image, true);
		imagesavealpha($this->tmp_image, true);
		
		$cut = imagecreatetruecolor($width+6, $height+6);
        // copying that section of the background to the cut
        imagecopy($cut, $this->tmp_image, 3, 3, 0, 0, $width, $height);
        // placing the watermark now
        imagecopy($cut, $border, 0, 0, 0, 0, $width+6, $height+6);
        //imagecopymerge($dst_im, $cut, $dst_x, $dst_y, $src_x, $src_y, $src_w, $src_h, 100);
		
		// Execute the crop
//		if ($status = imagecopymerge($this->tmp_image, $cut, 0, 0,0,0, $width+6, $height+6,100))
//		{
			// Swap the new image for the old one
			imagedestroy($border);
			imagedestroy($this->tmp_image);
...
//		}
    ...
    public function watermark($properties){
        ...
        imagecopy($img, $watermark, $properties['left'], $properties['top'], 0, 0, $properties['image']->width, $properties['image']->height);
        //imagejpeg($img, NULL, 80);
        $this->tmp_image = $img;
        return $img;
    }

    public function textWatermark($properties){
        $img = $this->tmp_image;
        $white = imagecolorallocate($img, 255, 255, 255);
        imagettftext($img, $properties['fontsize'], 0, $properties['left'], $properties['top'], $white, "css/arial.ttf", $properties['text']);
        $this->tmp_image = $img;
        return $img;
    }
    
    public function center_image($properties){
    	$new_img = imagecreatetruecolor( $properties['width'], $properties['height'] );
    	imagefilledrectangle ( $new_img, 0 , 0 , $properties['width'] , $properties['height'] , imagecolorallocate($new_img, 255,255,255));
		...
		$dst_x = $properties['width'] / 2 - $width / 2;
        $dst_y = $properties['height'] / 2 - $height / 2;
        
        imagecopy($new_img, $img, $dst_x, $dst_y, 0, 0, $width, $height);
        //imagejpeg($img, NULL, 80);
        $this->tmp_image = $new_img;
        return $new_img;
    }
} // End Image GD Driver